...
function defaultSchedule() {
  return {
    sat: [{ id: uid(), subject: 'عربي', time: '09:30' }, { id: uid(), subject: 'جيم', time: '16:00' }],
    sun: [{ id: uid(), subject: 'فيزياء', time: '09:30' }],
    mon: [{ id: uid(), subject: 'إنجليزي', time: '10:00' }],
    tue: [{ id: uid(), subject: 'كيمياء', time: '09:30' }, { id: uid(), subject: 'جيم', time: '16:00' }],
    wed: [{ id: uid(), subject: 'أحياء', time: '09:30' }],
    thu: [{ id: uid(), subject: 'إنجليزي', time: '10:00' }, { id: uid(), subject: 'جيم', time: '16:00' }],
    fri: [],
  };
}
...
function defaultState() {
  return {
    ...
    profileName: 'إبراهيم',
    goal: 'كلية الصيدلة 💊',
    examDate: null, // 'YYYY-MM-DD' — set by the user from Settings; never guessed
    scheduleV2Applied: true, // fresh installs already get the new schedule; only old saves need the migration
    autoTasksV2Purged: true, // fresh installs have nothing stale to purge
    xp: 0,
...
function loadState() {
  try {
    ...
    // one-time migration: replace the old weekly schedule with the new plan
    // Ibrahim sent (study block + gym days), without touching XP/tasks/logs.
    if (!parsed.scheduleV2Applied) {
      merged.schedule = defaultSchedule();
      merged.scheduleV2Applied = true;
    }
    // one-time cleanup: old auto-generated tasks (from before lessonId tracking
    // existed) never get removed on their own — purge every old auto task once
    // so today's list rebuilds cleanly from the current schedule.
    if (!parsed.autoTasksV2Purged) {
      merged.tasks = merged.tasks.filter(t => !t.auto);
      merged.autoTasksV2Purged = true;
      merged.lastAutoOtherDate = null; // force gym/reviews to regenerate today too
    }
    return merged;
  } catch (e) {
...
function examDaysLeft() {
  ...
  const exam = startOfDay(new Date(state.examDate));
  const now = startOfDay(new Date());
...
const MOTIVATIONS = [
  ...
  'كل مجهود بتبذله دلوقتي هيفرق يوم النتيجة.',
  'مفيش حد ولد وهو عارف كل حاجة — إنت بتتعلم وده كفاية.',
  'ثواني الشك دي، حولها لدقايق مذاكرة.',
  'اللي بتزرعه النهارده هتحصده في كشف الدرجات.',
  'مش لازم تكون في مزاجك عشان تذاكر، ابدأ وهيجيلك المزاج.',
  'كل امتحان صغير بتحله هنا بيقربك من امتحان الثانوية.',
  'راحتك دلوقتي مؤقتة، لكن ندمك لو قصّرت هيفضل.',
  'إنت مش بس بتذاكر، إنت بتبني نسخة أقوى منك.',
  'خطوة صغيرة النهارده أحسن من خطوة كبيرة متأجلة لبكرة.',
  'ركّز على المجهود، النتيجة هتيجي لوحدها.',
  'يوم صعب في المذاكرة؟ ده بالظبط اليوم اللي بيفرق.',
  'مفيش طريق مختصر للصيدلة غير المذاكرة المستمرة.',
  'إنت أقرب لحلمك من إمبارح بمرحلة.',
  'ابدأ الجلسة، وسيب باقي القلق برا الباب.',
  'كل دقيقة تركيز هنا هي استثمار في مستقبلك.',
  'التعب مؤقت، الشهادة والنجاح باقيين.',
  'إنت قدها، وإنت أثبتّ ده قبل كده كذا مرة.',
  'مفيش حد بيندم إنه ذاكر أكتر، بس ناس بتندم إنها ذاكرت أقل.',
  'خليك صبور مع نفسك، التفوق رحلة مش سباق.',
  'كل سؤال تحله صح دلوقتي، درع ليك يوم الامتحان.',
  'ثق في المجهود اللي بتعمله، حتى لو النتيجة لسه ما ظهرتش.',
  'إنت مش بعيد عن هدفك، إنت في نص الطريق بالظبط.',
  'ابدأ بجلسة واحدة، وخلي الباقي يتبني عليها.',
  'مذاكرتك النهارده هي رسالة لنفسك في يوم النتيجة.',
  'إنت بتستحق الصيدلة، وإنت بتشتغل عليها فعلاً.',
  'كل حاجة صعبة دلوقتي هتبقى سهلة بعد ما تتعود عليها.',
  'ثبات بسيط كل يوم أقوى من حماس يوم وكسل أسبوع.',
  'إنت مش لوحدك، التطبيق ده معاك في كل خطوة.',
  'كل يوم بتفضل مكمّل فيه، إنت بتثبت إنك تستاهل تحلم بحلم كبير.',
  'النهارده يوم تاني تقدر تبقى فيه أقرب لكلية الصيدلة — استغله.',
];
...
/* ===================== XP levels & titles (100 levels, one unique title each) ===================== */
const LEVEL_TITLES = [
  'بذرة الطموح 🌰', 'أول سطر في الدفتر ✏️', 'شرارة الانطلاق ✨', 'خطوة على الطريق 👣', 'حماس البداية 🔥',
  'التزام الأسبوع الأول 📅', 'عادة تتكوّن 🧩', 'تركيز يتشكّل 🎯', 'ثقة متزايدة 🌤️', 'طالب مجتهد 📖',
  'روتين يومي ثابت ⏰', 'مثابرة متصاعدة 📈', 'انضباط النفس 🛡️', 'جدول محكم 🗂️', 'مراجعة منتظمة 🔁',
  'أساس قوي 🧱', 'تقدّم ملموس 🚀', 'هدوء المذاكرة 🌙', 'صبر الطالب المجدّ 🕰️', 'باحث عن التفوق 🔍',
  'عمق في الفهم 🔬', 'أسئلة أذكى ❓', 'ربط المعلومات 🔗', 'مذاكرة بلا كلل 💪', 'استعداد جادّ 🎯',
  'تفوق يتشكّل 🏗️', 'نتائج أفضل 📊', 'ثقة الامتحانات 📝', 'استقرار الأداء ⚖️', 'متمكن من موادك 💪',
  'إتقان تدريجي 🧠', 'سرعة في الحل ⚡', 'دقة في الإجابة 🎯', 'مراجعة ذكية 🔁', 'استذكار عميق 📚',
  'نظام مذاكرة محكم 🗓️', 'تفوق ثابت 📈', 'جهد يُثمر 🌾', 'قرب من الهدف 🎯', 'مستعد للتفوق 🚀',
  'رؤية واضحة 🔭', 'خطة محكمة 🗺️', 'تركيز الماراثون 🏃', 'صبر البطل 🦾', 'نتائج تتحدث 📣',
  'ثقة الصيدلي القادم 💊', 'مذاكرة استراتيجية 🧭', 'قرب من القمة ⛰️', 'لحظة الحسم تقترب ⏳', 'على أعتاب الصيدلة 🎓',
  'خطوة قبل الأخيرة 🚶', 'عزم لا يلين 🔩', 'استعداد كامل 🎯', 'مراجعة شاملة 📖', 'ثقة تامة 💎',
  'هدوء المحترفين 🧘', 'تفوق مستمر 🌟', 'قرب النجاح 🏁', 'آخر الأشواط 🏃‍♂️', 'صيدلي المستقبل 💊',
  'عقلية الصيدلي 🧬', 'دقة علمية 🔬', 'مسؤولية متزايدة 🩺', 'معرفة تتوسّع 📗', 'ثقة الخبير 🧑‍🔬',
  'التزام لا يتزعزع 🛡️', 'أداء استثنائي 🌠', 'تميّز واضح ✨', 'قريب من القمة 🏔️', 'نجم المذاكرة ⭐',
  'إشعاع النجاح 🌟', 'تفوق يُحتذى 🏆', 'قدوة في الجد 🧭', 'إنجاز يتراكم 📦', 'مثابرة أسطورية 🔥',
  'عقل هادئ وقوي 🧠', 'ثقة لا تتزعزع 🗿', 'نجاح متكرر 🔁', 'قرب الذروة 🚩', 'أسطورة المذاكرة 🏆',
  'تاريخ يُكتب 📜', 'إرث من الجهد 🏛️', 'قوة لا تُقهر 💥', 'حكمة الطالب المخضرم 🦉', 'ثبات الأبطال 🛡️',
  'تفرّد حقيقي 🌌', 'قمة الانضباط ⛰️', 'إتقان نادر 💎', 'اقتراب من القمة الكبرى 🚀', 'أسطورة المذاكرة الكبرى 🏆',
  'عتبة الأسطورة 👑', 'إرادة لا تُهزم 🔱', 'تفوق خارق ⚡', 'مجد يتشكّل 🏵️', 'قرب الحلم الأكبر 💫',
  'خطوة من القمة 🏔️', 'صيدلي بامتياز 💊', 'نجاح يتوّج 🎖️', 'على أبواب المجد 🚪', 'أسطورة الصيدلة الكبرى 👑',
];
function generateLevels() {
  const levels = [];
  for (let n = 1; n <= 100; n++) {
    const title = LEVEL_TITLES[n - 1];
    // Level 2 needs 375 XP (~6h15m study at 1 XP/minute). Growth is very gentle —
    // level 100 (the max) is reachable in around 6.5 months of consistent daily study.
    const xp = n === 1 ? 0 : Math.round(375 * Math.pow(n - 1, 1.05));
...
let tickHandle = null;
let timerPerfAnchor = null; // monotonic clock reference (not persisted — resets each page load)
...
function startTimerSession(durationMinutes, subject) {
  state.timerSession = {
    ...
    reloadedAcrossPageLoad: false,
  };
  timerPerfAnchor = performance.now();
  saveState();
...
/* ---------- anti-tamper: catch a device clock being changed mid-session ----------
   Date.now() is what lets the timer keep counting correctly across backgrounding/
   sleep (the whole point of section 9), but that's exactly what someone could abuse
   by manually winding the clock forward to "finish" a session instantly. performance.now()
   is a monotonic clock the OS keeps ticking through real elapsed time (including sleep)
   but is immune to the user changing the wall-clock date/time. Comparing the two catches
   an artificial jump. This can only check the portion of a session that happened during
   the current page life — if the app was fully closed and reopened mid-session we can't
   verify the part before that, so we honestly skip the check rather than falsely accuse. */
function tamperCheckAndLoggedMinutes(session, wallElapsedMs) {
  if (session.reloadedAcrossPageLoad || timerPerfAnchor === null) {
    return { suspicious: false, minutes: Math.max(0, Math.round(wallElapsedMs / 60000)) };
  }
  const perfElapsedMs = performance.now() - timerPerfAnchor - session.pausedMs;
  const diff = Math.abs(wallElapsedMs - perfElapsedMs);
  if (diff > 90000) {
    const safeMs = Math.max(0, Math.min(wallElapsedMs, perfElapsedMs));
    return { suspicious: true, minutes: Math.max(0, Math.round(safeMs / 60000)) };
  }
  return { suspicious: false, minutes: Math.max(0, Math.round(wallElapsedMs / 60000)) };
}
function finishTimerSession(auto) {
  const s = state.timerSession;
  if (!s) return;
  const wallElapsedMs = elapsedMsOf(s);
  const { suspicious, minutes: elapsedMin } = tamperCheckAndLoggedMinutes(s, wallElapsedMs);
  state.timerSession = null;
  saveState();
  stopTicking();
  if (suspicious) {
    toast('⚠️ رصدنا تغييرًا غير معتاد في وقت الجهاز أثناء الجلسة — سجّلنا الوقت الفعلي المتحقق منه بس.', 4200);
  }
  if (elapsedMin > 0) {
...
function scheduleDailyNotifications() {
  ...
  if (state.settings.spacedReviewsEnabled && reviewMs > 0) {
    dailyNotifTimeouts.push(setTimeout(() => {
  ...
  // 4) last day of the month: one reminder per subject, at 9:30 AM
  const today = new Date();
  const isLastDayOfMonth = today.getDate() === new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
  if (isLastDayOfMonth) {
    const monthExamBaseMs = msUntilTodayAt(9, 30);
    if (monthExamBaseMs > 0) {
      SUBJECTS.forEach((subj, i) => {
        dailyNotifTimeouts.push(setTimeout(() => {
          sendLocalNotification('📅 نهاية الشهر', `موعد امتحان ${subj}`);
        }, monthExamBaseMs + i * 3000)); // stagger 3s apart so they don't overwrite each other
      });
    }
  }
}
...
function toggleTask(id) {
  ...
  if (t.done) {
    ...
    if (state.settings.spacedReviewsEnabled && t.auto && t.subject && t.subject !== 'الجيم' && !t.reviewId && !t.reviewsCreated) {
...
/* auto-generate/sync today's tasks from the schedule.
   Runs every render so that editing today's schedule (add/delete a lesson)
   is reflected immediately in today's task list, instead of only once per day. */
function ensureTodayAutoTasks() {
  const k = todayKey();
  const dayKey = jsDayToKey(new Date().getDay());
  const lessons = state.schedule[dayKey] || [];

  // remove auto lesson-tasks whose lesson was edited/removed from today's schedule
  const lessonIds = new Set(lessons.map(l => l.id));
  const beforeCount = state.tasks.length;
  state.tasks = state.tasks.filter(t => !(t.date === k && t.lessonId && !lessonIds.has(t.lessonId)));
  let changed = state.tasks.length !== beforeCount;

  // add a task for any lesson that doesn't have one yet
  lessons.forEach(lesson => {
    const exists = state.tasks.some(t => t.date === k && t.lessonId === lesson.id);
    if (!exists) {
      addTask({
        date: k, subject: lesson.subject, title: `مذاكرة ${lesson.subject}`,
        time: (lesson.time || '').split('–')[0].trim(), duration: 45, priority: 'عالية', auto: true, lessonId: lesson.id,
      });
      changed = true;
    }
  });

  // gym + spaced reviews: only need generating once per day (not schedule-edit dependent)
  if (state.lastAutoOtherDate !== k) {
    const gym = isGymDayToday();
    if (gym === 'gym') {
      addTask({ date: k, subject: 'الجيم', title: 'الجيم 🏋️', time: '', duration: 60, priority: 'متوسطة', auto: true });
    }
    if (state.settings.spacedReviewsEnabled) {
...
function renderHome() {
  ...
  <div class="screen">
    ...
    <div class="card card-navy timer-card">
      ...
      <div class="timer-display" id="timerDisplay">${s ? fmtHMS(Math.max(0, (s.plannedMs > 0 ? (s.plannedMs - elapsedMsOf(s)) : elapsedMsOf(s)) / 1000)) : '00:00:00'}</div>
      <div class="timer-sub" id="timerSub">${s ? (s.plannedMs > 0 ? 'الوقت المتبقي' : 'وقت مفتوح — بدون حد') : 'كل دقيقة هنا تقربك من هدفك'}</div>
      ${!s ? `
        <div class="timer-durations" id="durationPicker">
          ...
          <span class="dur-chip" data-dur="open">⏱️ وقت مفتوح</span>
        </div>
    ...
    ${state.settings.spacedReviewsEnabled && todaysReviews().length ? `
...
function renderExamCountdownCard() {
  if (!state.examDate) {
    return `<div class="card" style="cursor:pointer;" data-nav="more">
      <p style="margin:0;font-size:13px;color:var(--muted);">📅 حدد موعد امتحان الثانوية العامة من "المزيد" عشان يظهرلك هنا عداد تنازلي.</p>
    </div>`;
  }
  const days = examDaysLeft();
  let text, emoji;
  if (days > 1) { emoji = '🎯'; text = `باقي <b>${days}</b> يوم على امتحان الثانوية العامة`; }
  else if (days === 1) { emoji = '🔥'; text = `باقي يوم واحد بس! جهّز نفسك يا إبراهيم`; }
  else if (days === 0) { emoji = '💪'; text = `النهارده يوم الامتحان — إنت جاهز`; }
  else { emoji = '🎉'; text = `خلصت امتحاناتك! مبروك يا إبراهيم`; }
  return `<div class="card card-coral">
    <div class="msg-row">
      <div class="msg-icon">${emoji}</div>
      <div><p class="msg-text" style="margin:0;">${text}</p></div>
    </div>
  </div>`;
}
...
function renderMore() {
  ...
  <div class="screen">
    ...
    <div class="card">
      ...
      <div class="switch-row" style="border-top:1px solid var(--line);margin-top:6px;padding-top:14px;border-bottom:none;">
        <span class="switch-label">🧠 المراجعات المتباعدة</span>
        <div class="switch ${s.spacedReviewsEnabled ? 'on' : ''}" id="spacedReviewsSwitch"><div class="knob"></div></div>
      </div>
      <p style="font-size:11px;color:var(--muted);margin:6px 2px 0;line-height:1.7;">
        لو قفّلتها: هتوقف رسالة "مراجعات مستحقة اليوم" وإشعارها، ومهام المراجعة مش هتتضاف تلقائي لقائمة مهامك.
      </p>
    </div>
    <div class="card">
      <label>📅 الجدول</label>
      <p style="font-size:11.5px;color:var(--muted);margin:0 0 10px;line-height:1.7;">لو الجدول لسه فيه حصص قديمة بتظهر معاك رغم إنك مسحتها، دوس الزرار ده عشان يمسح كل الجدول ويرجّعه للخطة الافتراضية (عربي، فيزياء، إنجليزي، كيمياء، أحياء + أيام الجيم)، وابدأ تضيف حصصك من "الجدول" تاني.</p>
      <button class="btn-secondary" id="wipeScheduleBtn">🗑️ امسح الجدول القديم وابدأ من جديد</button>
    </div>
    <div class="card">
    ...
    <div class="card">
      <label>📅 موعد امتحان الثانوية العامة</label>
      <input type="date" id="examDateInput" value="${state.examDate || ''}">
      <button class="btn-secondary" id="saveExamDateBtn">حفظ الموعد</button>
    </div>
    <div class="card">
...
function bindScreenEvents() {
  ...
  $$('#durationPicker .dur-chip').forEach(chip => chip.addEventListener('click', () => {
    ...
    } else if (chip.dataset.dur === 'open') {
      $$('#durationPicker .dur-chip').forEach(c => c.classList.remove('active'));
      chip.classList.add('active');
      selectedDuration = 0; // 0 = open-ended, counts up with no limit until you finish it
    } else {
      $$('#durationPicker .dur-chip').forEach(c => c.classList.remove('active'));
      chip.classList.add('active');
  ...
  const spacedReviewsSwitch = $('#spacedReviewsSwitch');
  if (spacedReviewsSwitch) spacedReviewsSwitch.addEventListener('click', () => {
    state.settings.spacedReviewsEnabled = !state.settings.spacedReviewsEnabled;
    if (!state.settings.spacedReviewsEnabled) {
      // remove any not-yet-done auto review tasks already sitting in today's list
      state.tasks = state.tasks.filter(t => !t.reviewId || t.done);
    }
    saveState(); render(); scheduleDailyNotifications();
  });
  const wipeScheduleBtn = $('#wipeScheduleBtn');
  if (wipeScheduleBtn) wipeScheduleBtn.addEventListener('click', () => {
    state.schedule = defaultSchedule();
    // also drop today's auto lesson-tasks so they rebuild from the fresh schedule
    state.tasks = state.tasks.filter(t => !t.auto || t.date !== todayKey());
    state.lastAutoOtherDate = null;
    saveState(); toast('تم مسح الجدول القديم'); render(); scheduleLessonReminders(); scheduleDailyNotifications();
  });
  ...
  const saveExamDateBtn = $('#saveExamDateBtn');
  if (saveExamDateBtn) saveExamDateBtn.addEventListener('click', () => {
    const v = $('#examDateInput').value;
    state.examDate = v || null;
    saveState(); toast(v ? 'تم حفظ موعد الامتحان' : 'تم مسح موعد الامتحان'); render();
  });
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      // Just register — the service worker now caches pages as they're opened
      // online (network-first, cache fallback), so the app works offline once
      // it's been opened with internet at least one time. We no longer wipe
      // the cache on every load, since that was what made the app require
      // internet every single time.
...
applyDarkMode();
if (state.timerSession && !state.timerSession.reloadedAcrossPageLoad) {
  // A session survived a full page reload/app relaunch — we can't verify the
  // pre-reload portion against the monotonic clock, so mark it unchecked rather
  // than risk a false tamper accusation. See tamperCheckAndLoggedMinutes above.
  state.timerSession.reloadedAcrossPageLoad = true;
  timerPerfAnchor = performance.now();
  saveState();
}
render();
